...
const toPath = (c: Category) =>
  c === 'Clothes' ? '/sau/merchandise/create' :
  c === 'Stationary' ? '/sau/merchandise/create/stationary' :
  '/sau/merchandise/create/accessory';

function genNo() { return `M${Math.floor(100000 + Math.random()*900000)}`; }

export default function SAUCreateMerchAccessoryPage() {
  const router = useRouter();
  const [no, setNo] = useState(''); useEffect(() => setNo(genNo()), []);
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6">
      ...
      <form onSubmit={onSubmit} className="mt-4 space-y-4">
        ...
        <Row label="Merchandise Number"><div className="py-2 font-mono">{no || '—'}</div><input type="hidden" name="merchNumber" value={no}/></Row>
        ...
        <div className="rounded-lg border border-zinc-200 p-3">
          <div className="text-sm font-semibold mb-2">Option 1</div>
        ...
        <Row label="Special Price">
          <div className="flex items-center gap-6">
            <label className="flex items-center gap-2"><input type="radio" name="special" checked={special==='yes'} onChange={()=>setSpecial('yes')}/><span>Yes</span></label>
            <label className="flex items-center gap-2"><input type="radio" name="special" checked={special==='no'} onChange={()=>setSpecial('no')}/><span>No</span></label>
          </div>
        </Row>
        <Field label="Quantity" name="specialQty" type="number" min={0} disabled={disabled} className={disabled?'opacity-50 pointer-events-none':''}/>
        <Field label="Percentage" name="specialPct" type="number" min={0} disabled={disabled} className={disabled?'opacity-50 pointer-events-none':''}/>
...
function Field({ label, name, type='text', min, disabled, className }:{
  label: string; name: string; type?: string; min?: number; disabled?: boolean; className?: string;
}) {
  return (
    <Row label={label}>
      <input name={name} type={type} min={min} disabled={disabled}
